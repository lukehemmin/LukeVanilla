...
class AdvancedLandSystem(
    ...
    private lateinit var villageSettingsGUI: VillageSettingsGUI
    ...
    fun enable() {
        try {
            ...
            villageSettingsGUI = VillageSettingsGUI(plugin, advancedLandManager)
            plugin.server.pluginManager.registerEvents(villageSettingsGUI, plugin)
            debugManager.log("AdvancedLandClaiming", "VillageSettingsGUI 등록 완료")
    ...
    fun integrateWithLandCommand(landCommand: LandCommand, landManager: com.lukehemmin.lukeVanilla.System.MyLand.LandManager) {
        if (::advancedLandManager.isInitialized && ::villageSettingsGUI.isInitialized) {
            // AdvancedLandManager → LandCommand 연결
            landCommand.setAdvancedLandManager(advancedLandManager)

            // VillageSettingsGUI → LandCommand 연결
            landCommand.setVillageSettingsGUI(villageSettingsGUI)

            // AdvancedLandManager → LandManager 연결 (연결된 청크 그룹 시스템을 위해)
            advancedLandManager.setLandManager(landManager)

            debugManager.log("AdvancedLandClaiming", "LandCommand, LandManager 및 VillageSettingsGUI와 통합 완료")
        } else {
            plugin.logger.warning("[AdvancedLandClaiming] AdvancedLandManager 또는 VillageSettingsGUI가 초기화되지 않았습니다. integrateWithLandCommand 호출을 연기하세요.")
        }
    }
    ...
    /**
     * VillageSettingsGUI를 반환합니다.
     */
    fun getVillageSettingsGUI(): VillageSettingsGUI? {
        return if (::villageSettingsGUI.isInitialized) {
            villageSettingsGUI
        } else {
            null
        }
    ...
    fun isInitialized(): Boolean {
        return ::advancedLandManager.isInitialized &&
               ::advancedLandProtectionListener.isInitialized &&
               ::villageSettingsGUI.isInitialized
    }
